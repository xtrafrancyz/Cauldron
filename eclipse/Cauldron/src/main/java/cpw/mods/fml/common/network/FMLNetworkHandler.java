    ...
    private void handleClientConnection(NetLoginHandler netLoginHandler, MinecraftServer server, SocketAddress address, String userName)
    {
        if (!loginStates.containsKey(netLoginHandler))
        {
            if (handleVanillaLoginKick(netLoginHandler, server, address, userName))
            {
                ...
                netLoginHandler.completeConnection(net.minecraftforge.cauldron.configuration.CauldronConfig.getString("settings.fml-missing-message", "You do not have FML installed, you cannot connect to this server")); // Cauldron
                return;
            }
            else
            {
        ...
        case MISSING_MODS_OR_VERSIONS:
            netLoginHandler.completeConnection(net.minecraftforge.cauldron.configuration.CauldronConfig.getString("settings.fml-missing-mods-message", "The server requires mods that are absent or out of date on your client")); // Cauldron
            loginStates.remove(netLoginHandler);
            break;
        case FML_OUT_OF_DATE:
            netLoginHandler.completeConnection(net.minecraftforge.cauldron.configuration.CauldronConfig.getString("settings.fml-outdated-message", "Your client is not running a new enough version of FML to connect to this server")); // Cauldron
            loginStates.remove(netLoginHandler);
            break;
        default:
            netLoginHandler.completeConnection(net.minecraftforge.cauldron.configuration.CauldronConfig.getString("settings.fml-other-error-message", "There was a problem during FML negotiation")); // Cauldron
            loginStates.remove(netLoginHandler);
            break;
        }
    }
